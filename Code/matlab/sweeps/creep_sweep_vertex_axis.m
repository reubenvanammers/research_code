...
%subplot, for the vertex model with added axial component.
clear all
...
%save([pwd '/workspaces/creeperrorvertexaxis' num2str(Tvec(1)) '_' num2str(Tvec(end)) '_' num2str(etavec(1)) '-' num2str(etavec(end)) '_' num2str(alphavec(1)) '-' num2str(alphavec(end)) '.mat']);
%%
%load([pwd '/workspaces/creeperrorvertexaxis' num2str(Tvec(1)) '_' num2str(Tvec(end)) '_' num2str(etavec(1)) '-' num2str(etavec(end)) '_' num2str(alphavec(1)) '-' num2str(alphavec(end)) '.mat']);
...
%     alpha(0.5);
    ...
    SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/timeendsurface/' num2str(Tvec(T_value)) '-' num2str(forcevec(force_index))]  , 7, 7/5, 9)
...
%             h(8) = plot(etavec(eta_index),-fit1(vars{:},2),'rx','markers',5);
            ...
            xlabel('\eta')
            ylabel('Time coefficients')
            SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/lineplot/' num2str(Tvec(T_value)) '-' num2str(forcevec(force_index)) '-alpha=' num2str(alphavec((alpha_index-1)*a_scale+1))]  , 7, 7/5, 9)
            %legend(h)
    end
end
%%
...
%             h(8) = plot(alphavec(alpha_index),-fit1(vars{:},2),'rx','markers',5);
            ...
            xlabel('\alpha')
            ylabel('Time coefficients')
            SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/lineplot/' num2str(Tvec(T_value)) '-' num2str(forcevec(force_index)) '-eta=' num2str(etavec((eta_index-1)*g_scale+1))]  , 7, 7/5, 9)
            %legend(h)
    end
end
%%
...
for force_index = 1:f
    for T_index = 1:length(Tvec)
        ...
        colorbar;
        caxis([1,3])
        SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/timedifsurface/' num2str(Tvec(T_index)) '-' num2str(forcevec(force_index))]  , 7, 7/5, 9)
    end
end
...
for T_index = 1:length(Tvec)
    ...
    SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/biexpcontour/' num2str(Tvec(T_index))]  , 7, 7/5, 9)
...
%     Tstring2 ={Tstring{1} Tstring{4} Tstring{6}};
    ...
    SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/biexpcontour2/' num2str(forcevec(force_index))]  , 7, 7/5, 9)
...
xlabel('Time')
ylabel('Strain')
SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/timestrain_threeexp/' num2str(Tvec(T_value)) '-' num2str(forcevec(force_index)) '-' num2str(etavec(eta_index)) '-' num2str(alphavec(alpha_index))]  , 7, 7/5, 9)
%pause
...
time = timecell3{vars{:}}; strain = straincell3{vars{:}};
plot(timecell3{vars{:}},strain-exp1(timecell3{vars{:}}),'k-',timecell3{vars{:}},strain-exp2(timecell3{vars{:}}),'b--',timecell3{vars{:}},strain-exp3(timecell3{vars{:}}),'g-.','MarkerSize',4)
%plot(timecell3{vars{1:end-1}},straincell3{vars{1:end-1}},'r',timecell3{vars{1:end-1}},exp1(timecell3{vars{1:end-1}}),'k--',timecell3{vars{1:end-1}},exp2(timecell3{vars{1:end-1}}),'b--',timecell3{vars{1:end-1}},exp2_1(timecell3{vars{1:end-1}}),'g-.',timecell3{vars{1:end-1}},exp2_2(timecell3{vars{1:end-1}}),'y-.')
title([astring{alpha_index}, estring{eta_index}, fstring{force_index}, Tstring{T_value} ]); 
%legend('Data','Single Exp', 'Two Exp', 'Short Time', 'Long Time') 
%axis([0 1 0 1]);
...
SaveAsPngEpsAndFig(-1,[pwd '/pictures/expfitvertexaxis/creep/timestrain_diff/' num2str(Tvec(T_value)) '-' num2str(forcevec(force_index)) '-' num2str(etavec(eta_index)) '-' num2str(alphavec(alpha_index))]  , 7, 7/5, 9)
%pause
